...
###
CORR<-array(NA,c(n.run,1,4,n.prs)) 
###
###############################################################
###############################################################
###############################################################
iter<-1;h<-4 # for example, 8000 causal SNPs
###############################################################
###############################################################
###############################################################
cat(" - Start simulation.\n")
for(h in 1:length(n.null.snps)){
  set.seed(12345+2000*h+200) 
  ###############################################################
  ...
  for (iter in 1:n.run){if(iter %%10==0) {cat(paste(" - Simulation Iter: ",iter,"/",n.run,".",sep=""))}
    set.seed(12345+iter+2000*h+200) 
    #####
    ...
    #####
    etrain.temp<-rnorm(n.train,mean=0,sd=1)
    etest.temp<-rnorm(n.test,mean=0,sd=1)
    #####
    signal.train<-scale(x.train.causal%*%beta1)
    signal.test<-scale(x.test.causal%*%beta2)
    ####################################heritability is set to 50%
...
cols<-c('orange','yellow','green','red','sienna','palevioletred1','royalblue2','darkturquoise',
        'darkviolet','brown','midnightblue','dimgrey','darkorange1',"forestgreen")
xtitles<-c("Cutoffs")
prs.cutoffs<-c(1,0.8,0.5,0.4,0.3,0.2,0.1,0.08,0.05,0.02,0.01,
               0.001,0.0001,0.00001,0.000001,0.0000001)
name.pred<-list()
...
##################
pdf("Figure-2.pdf",width=15,height=5)
########################
...
for(k in 1:4){
  ...
  mtext(paste(name.pred[[2]][k],sep = ""), side = 3, line = 1,cex=1.2)
  ##
